...
export class PlayerController {
  ...
  /**
   * Movement/look-delta processing only runs while pointer-locked (real
   * play); the camera transform itself is always synced from state so a
   * fresh spawn — or an automated test driving `position`/`yaw` directly
   * without ever acquiring the OS pointer lock — still renders correctly.
   */
  update(dt, { processInput = true } = {}) {
    if (processInput) {
